...
import { getGitHubRepositorySlug } from "@/lib/github-star"
      ...
      <PanelContent className="p-0">
        <div className="divide-y divide-line">
          {repositories.map((project, index) => (
            <ProjectItem key={project.id} project={project} open={index < 2} />
          ))}
        </div>
      </PanelContent>
    </Panel>
...
function ProjectItem({
  ...
  return (
    ...
    >
      <summary className="flex min-w-0 cursor-pointer list-none items-center hover:bg-muted/50 focus-visible:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset [&::-webkit-details-marker]:hidden">
        ...
        <ChevronDown
          className="mr-4 size-4 shrink-0 text-muted-foreground transition-transform duration-150 ease-out group-open/project:rotate-180 motion-reduce:transition-none"
          aria-hidden="true"
        />
      </summary>
      ...
      <div className="grid gap-4 border-t border-line p-4 sm:grid-cols-[1fr_14rem] sm:p-5">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <a className="panel-link" href={project.url} {...externalLinkProps}>
              Repository
            ...
            <StarProjectButton
              ...
              repository={getGitHubRepositorySlug(project.url)}
        ...
        <div className="relative aspect-[1200/630] overflow-hidden rounded-xl border border-line bg-muted">
          <Image
            ...
            unoptimized
            sizes="224px"
            className="object-cover opacity-90 transition-opacity duration-300 ease-out group-hover/project:opacity-100 motion-reduce:transition-none"
          />
        </div>
      </div>
    </details>
